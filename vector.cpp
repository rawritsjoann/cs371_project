...
void makeUnitNormalFromVerts(Vec tp[], Vec norm)
{
	Flt v1 = tp[0][0] - tp[1][0];
	Flt v2 = tp[0][1] - tp[1][1];
	Flt v3 = tp[0][2] - tp[1][2];
	Flt w1 = tp[2][0] - tp[1][0];
	Flt w2 = tp[2][1] - tp[1][1];
	Flt w3 = tp[2][2] - tp[1][2];
	//calculate perpendicular relative to the BaseIndex point in Vec
	Flt xlen = v2*w3 - v3*w2;
	Flt ylen = v3*w1 - v1*w3;
	Flt zlen = v1*w2 - v2*w1;
	Flt len = xlen*xlen + ylen*ylen + zlen*zlen;
	if (len == 0.0) {
		MakeVector(0,0,1,norm);
		return;
	}
	len = 1.0 / sqrt(len);
	norm[0] = xlen * len;
...
void makeUnitNormalFromVerts3(Vec p0, Vec p1, Vec p2, Vec norm)
{
	Flt tlen;
	Flt v1 = p0[0] - p1[0];
	Flt v2 = p0[1] - p1[1];
...
void makeNormalFromVerts(Vec tp[], Vec norm)
{
	Flt v1, v2, v3, w1, w2, w3;
	v1 = tp[0][0] - tp[1][0];
	v2 = tp[0][1] - tp[1][1];
	v3 = tp[0][2] - tp[1][2];
	w1 = tp[2][0] - tp[1][0];
...
void matrixCat(Matrix m1, Matrix m2, Matrix dest)
{
...
void transVector(Matrix mat, Vec in, Vec out)
{
...
void transNormal(Matrix mat, Vec in, Vec out)
{
	//transform a normal
...
void makeViewMatrix(Vec p1, Vec p2, Matrix m)
{
	Flt d, len, l1;
	Vec up, out, left;
	Vec upv={0,1,0};
